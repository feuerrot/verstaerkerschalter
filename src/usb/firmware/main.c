...
volatile uint8_t relaistatus;

static void outputByte(){
    PORTB = (PORTB & ~0x0f) | (relaistatus & 0x0f);
}
...
int main(void){
    ...
    wdt_enable(WDTO_2S);
    DDRB = ~(1<<PB6);
    PORTB = (1<<PB4);          /* no pullups on USB pins */
    DDRD = ~((1<<PD2) | (1<<PD3) | (1<<PD5));   /* all outputs except PD2 = INT0 */
    PORTD = (1<<PD5);
